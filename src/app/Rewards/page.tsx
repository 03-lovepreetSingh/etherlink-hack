...
import {useWaitForTransactionReceipt  , useReadContract, useEstimateGas, useAccount, useWriteContract, usePublicClient } from "wagmi";
const abi=[
	{
		"inputs": [],
		"name": "deposit",
...
export default function Rewards() {
  ...
  const [customWithdrawlAmount, setCustomWithdrawlAmount] = useState<number | null>(null);
  const { address, isConnected } = useAccount();
  const [depositAmount, setDepositAmount] = useState('');
  const [forwardToAddress, setForwardToAddress] = useState('');
  const [forwardAmount, setForwardAmount] = useState('');
  const { data: forwardHash, isPending: isForwarding, writeContract, error: forwardError } = useWriteContract();
  const { isLoading: isConfirmingForward, isSuccess: isForwardConfirmed } = useWaitForTransactionReceipt({ hash: forwardHash });
  ...
  useEffect(() => {
    const fetchUser = async() => {
        ...
        console.log(responseData,"Userss")
    }
    fetchUser();
  },[session])
  useEffect(() => {
  console.log(form.getValues("amount"),"test amt")

  },[control])
 const handleWithdrawal = () => {
  try {
    const netamtavailable:number =  parseFloat(totalRewarded.toFixed(4)) - parseFloat(totalPaidRewards.toFixed(4))
...
const doWithdrawal = async() => {
  ...
  try {
    ...
    console.log('totalrewardedinwei',totalRewardedWei)
    console.log(" userData[0]?.metaMask", userData[0]?.metaMask)
    writeContract({
      abi,
      address: userData[0]?.metaMask,
...
const doCustomWithdrawal = async(amountWithdrawn:number) => {
  ...
  useEffect(() => {
    const fetchRewards = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/rewards?contributor=${session?.user?.username}`);
    ...
    const fetchPayments = async() => {
      try{
        const response= await fetch(`/api/payments?username=${session?.user?.username}`, {method: 'GET',});
        ...
        console.log(totalPaidReward,"paid Rewards")
  ...
  const filteredRewards = rewards.filter((reward) => {
    ...
    if (currentUsername) {
      return (
        reward.Contributor.toLowerCase() === currentUsername.toLowerCase()
      );
    }
  ...
  useEffect(() => {
      ...
      const payment = async() => {
        await fetch('/api/payments', {
          ...
          body: JSON.stringify({
            ...
            amount: customWithdrawlAmount ? customWithdrawlAmount.toFixed(4) : totalRewarded.toFixed(4),
   ...
   useEffect(() => {
    const data= parseFloat(totalRewarded.toFixed()) - parseFloat(totalPaidRewards.toFixed())
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        ...
                        <p className="text-xl lg:text-3xl font-bold text-neutral-900 dark:text-white">
                           {parseFloat(totalPaidRewards.toFixed(4))} PHAROS
                        </p>
                      </div>
                ...
                <CardContent className="p-0">
                  ...
                  {!error && rewards.length > 0 && (
                    <>
                      ...
                      withdraw ?
                      <>
                       <div className={`mt-12 h-screen border-l-1 border-neutral-800
                          ${!withdraw ? "w-0" : isMobile ? "w-full fixed top-0 left-0 z-50 bg-white dark:bg-black overflow-y-auto" : "w-[30%]"}
                        `}>
                            {isMobile && (
                              <div className="p-4 flex justify-end">
                                <Button variant="ghost" onClick={() => setWithdraw(false)} className="text-neutral-500">
                            ...
                            <div className="p-4 md:p-8 border-b-1 border-neutral-900">
                                   ...
                                   {address && (
                                      <div className="flex items-center gap-1">
                                        ...
                                        <button 
                                          ...
                                          title="Copy address"
                                        >
                                          <Icon icon="heroicons:clipboard" width="16" height="16" />
                                        </button>
                                      </div>
                                    )}
                                    </div>
                              ...
                              <div className="text-sm mt-2">
                                You will receive payments in this wallet solve an issue. Learn more about what you can do with your rewards.
                              </div>
                            </div>
                            <div className="p-4 md:p-8">
                              <div className="flex flex-wrap items-center text-2xl md:text-3xl font-bold">
                                <img src="/pharos_small.png" alt="Pharos" width={32} height={32} className="mr-2" />
                                <span className="break-all">{parseFloat(totalRewarded.toFixed(4)) - parseFloat(totalPaidRewards.toFixed(4))}</span>
                                <div className="text-lg md:text-xl text-neutral-400 dark:text-neutral-600 pl-2 my-auto">Pharos</div>
                              </div>
                              ...
                              <div className="pt-4">
                                  ...
                                  >
                                    Withdraw All
                                    <Icon icon="iconamoon:arrow-top-right-1-bold" width="24" height="24" className="ml-2" />
                                  </Button>
                                  {isConfirmingForward && (
                                    <Alert className="mt-4">
                                        ...
                                        <AlertDescription>
                                            Your withdrawal transaction is being processed.
                                        </AlertDescription>
                                    </Alert>
                                )}
                            ...
                            <div className="border-t-1 border-neutral-900">
                                 <div className="p-4 md:p-8">
                                    ...
                                    <div className="text-sm">
                                      Your crypto, your control. Withdraw the exact amount you need, every time.
                                    </div>
                                ...
                                <Button className="w-full md:w-auto md:ml-2 flex justify-center items-center" 
                                  onClick={() => {doCustomWithdrawal(form.getValues("amount"))}}
                                >  
                                  Withdraw
                                  <Icon icon="iconamoon:arrow-top-right--bold" width="16" height="16" className="ml-1" />
                                </Button>
                              </div>
                              
                              <div className="flex flex-wrap gap-2 pt-2">
                                <Button onClick={() => form.setValue('amount', 20)} className="flex flex-1 min-w-[80px] justify-center dark:hover:bg-neutral-800 hover:bg-neutral-200 dark:bg-neutral-900 dark:text-neutral-100 bg-neutral-100 text-neutral-900">
                                  <img src="/pharos_small.png" alt="Pharos" width={16} height={16} className="mr-1" />
                                  20
                                </Button>
                                <Button onClick={() => form.setValue('amount', 50)} className="flex flex-1 min-w-[80px] justify-center dark:hover:bg-neutral-800 hover:bg-neutral-200 dark:bg-neutral-900 dark:text-neutral-100 bg-neutral-100 text-neutral-900">
                                  <img src="/pharos_small.png" alt="Pharos" width={16} height={16} className="mr-1" />
                                  50
                                </Button>
                                <Button onClick={() => form.setValue('amount', 100)} className="flex flex-1 min-w-[80px] justify-center dark:hover:bg-neutral-800 hover:bg-neutral-200 dark:bg-neutral-900 dark:text-neutral-100 bg-neutral-100 text-neutral-900">
                                  <img src="/pharos_small.png" alt="Pharos" width={16} height={16} className="mr-1" />
                                  100
                                </Button>
                              </div>  
                       ...
                       {isMobile && withdraw && (
                         <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setWithdraw(false)}></div>
                       )}
                      </>:
                      null
                    }
